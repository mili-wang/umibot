...
如果你之前安装过 umibot 插件，但不熟悉 `openclaw plugins` 升级命令或 `npm` 操作，建议优先使用项目内置脚本。
...
# 常见历史插件 ID：umibot / umibot
# 对应 npm 包：@sliverp/umibot / @mili-wang/umibot
openclaw plugins uninstall umibot
openclaw plugins uninstall umibot

# 如果你还安装过其它 umibot 相关插件，也请一并 uninstall
