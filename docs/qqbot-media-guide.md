...
STT 支持两级配置，按优先级查找：`channels.umibot.stt`（插件专属） → `tools.media.audio.models[0]`（框架级回退）。以框架级配置为例：
...
AI 在回复中写 `<umiimg>` 标签，系统自动上传发送。用户说"画个猫咪"，AI 就能调用绘图工具生成图片，直接发到对话里：

> **你**：画一只猫咪
>
> **UMIBot**：`<umiimg>~/.openclaw/umibot/images/cute-cat.png</umiimg>`
...
TTS 同样支持两级配置：`channels.umibot.tts`（插件专属） → `messages.tts`（框架级回退）。以插件专属配置为例：
...
> **你**：用语音讲个笑话
>
> **UMIBot**：`<umivoice>~/.openclaw/umibot/tts/joke.silk</umivoice>`
...
>
> **UMIBot**：`<umifile>~/.openclaw/umibot/downloads/战争与和平_第一章.txt</umifile>`
...
> **你**：发一个演示视频给我
>
> **UMIBot**：`<umivideo>~/.openclaw/umibot/downloads/demo.mp4</umivideo>`
...
| 发送文件 | "帮我生成一个文件" | 生成文件并通过 `<umifile>` 发送 |
| 发送视频 | "发个视频给我" | 通过 `<umivideo>` 直接发送视频 |
...
- **标签容错**：`<umi_img>`、`<image>`、甚至中文尖括号 `＜umiimg＞` 等 30 多种变体写法均可自动纠正为标准标签
...
发送  图片 → <umiimg> 标签，本地文件/URL 均可
      语音 → <umivoice> 标签，TTS 合成或已有音频
      文件 → <umifile> 标签，任意格式最大 20MB
      视频 → <umivideo> 标签，本地文件/URL
```
