...
- **Windows 升级脚本**（PR #130 by [@anyangmvp](https://github.com/anyangmvp)）：新增 `scripts/upgrade.bat`，为 Windows 用户提供与 `upgrade.sh` 等价的一键升级脚本，自动清理旧插件目录和配置文件中的 umibot 字段，并输出重新安装指引。
...
- **Windows upgrade script** (PR #130 by [@anyangmvp](https://github.com/anyangmvp)): Added `scripts/upgrade.bat` as a Windows equivalent to `upgrade.sh`. It automatically cleans up the old plugin directory and removes umibot-related fields from the config file, then prints reinstall instructions.
