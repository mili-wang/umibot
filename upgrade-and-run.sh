...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo ""
            echo "选项:"
            echo "  --appid <appid>       Umi机器人 AppID"
            echo "  --secret <secret>     Umi机器人 Secret"
            echo "  --markdown <yes|no>   是否启用 Markdown 消息格式（默认: no）"
            echo "  -h, --help            显示帮助信息"
            echo ""
            echo "也可以通过环境变量设置:"
            echo "  UmiBOT_APPID           Umi机器人 AppID"
            echo "  UmiBOT_SECRET          Umi机器人 Secret"
            echo "  UmiBOT_TOKEN           Umi机器人 Token (AppID:Secret)"
            echo "  UmiBOT_MARKDOWN        是否启用 Markdown（yes/no）"
            echo ""
...
# 使用命令行参数或环境变量
APPID="${APPID:-$UMIBOT_APPID}"
SECRET="${SECRET:-$UMIBOT_SECRET}"
MARKDOWN="${MARKDOWN:-$UMIBOT_MARKDOWN}"

echo "========================================="
echo "  Umi Bot 一键更新启动脚本"
echo "========================================="
...
for APP_NAME in openclaw clawdbot; do
    ...
    if [ -f "$CONFIG_FILE" ]; then
        SAVED_UMIBOT_TOKEN=$(node -e "
            const cfg = JSON.parse(require('fs').readFileSync('$CONFIG_FILE', 'utf8'));
            const ch = cfg.channels && cfg.channels.umibot;
        ...
        " 2>/dev/null || true)
        if [ -n "$SAVED_UMIBOT_TOKEN" ]; then
            echo "已备份 umibot 通道 token: ${SAVED_UMIBOT_TOKEN:0:10}..."
            break
        fi
    fi
done
...
if [ -n "$APPID" ] && [ -n "$SECRET" ]; then
    UMIBOT_TOKEN="${APPID}:${SECRET}"
    echo "使用提供的 AppID 和 Secret 配置..."
    echo "配置机器人通道: umibot"
    echo "使用Token: ${UMIBOT_TOKEN:0:10}..."

    if ! openclaw channels add --channel umibot --token "$UMIBOT_TOKEN" 2>&1; then
        echo ""
        ...
        echo "3. umibot通道已存在"
        echo ""
        echo "您可以稍后手动配置: openclaw channels add --channel umibot --token 'AppID:Secret'"
    else
        echo "✅ 机器人通道配置成功"
    fi
elif [ -n "$UMIBOT_TOKEN" ]; then
    echo "使用环境变量 UMIBOT_TOKEN 配置..."
    echo "使用Token: ${UMIBOT_TOKEN:0:10}..."
    ...
    if [ -n "$SAVED_UMIBOT_TOKEN" ]; then
        ...
        if ! openclaw channels add --channel umibot --token "$SAVED_UMIBOT_TOKEN" 2>&1; then
            echo "⚠️  警告: 恢复通道配置失败，可能通道已存在"
        else
            echo "✅ 已从备份恢复 umibot 通道配置"
        fi
    else
...
if [ -n "$MARKDOWN" ]; then
    ...
    if openclaw config set channels.umibot.markdownSupport "$MARKDOWN_VALUE" 2>&1; then
        echo "✅ Markdown配置成功"
    else
        ...
        if [ -f "$OPENCLAW_CONFIG" ] && node -e "
          ...
          cfg.channels.umibot.markdownSupport = $MARKDOWN_VALUE;
