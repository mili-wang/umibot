...
for APP_NAME in openclaw clawdbot moltbot; do
    ...
    if [ -z "$SAVED_MARKDOWN" ]; then
        SAVED_MARKDOWN=$(node -e "
            const cfg = JSON.parse(require('fs').readFileSync('$CONFIG_FILE', 'utf8'));
            const v = (cfg.channels && cfg.channels.umibot && cfg.channels.umibot.markdownSupport);
            if (v !== undefined && v !== null) process.stdout.write(String(v));
        " 2>/dev/null || true)
    fi

    [ -n "$SAVED_UMIBOT_TOKEN" ] && [ -n "$SAVED_MARKDOWN" ] && break
done
...
TMP_DIR="$PROJ_DIR/.umibot-update-tmp"
...
for APP_NAME in openclaw clawdbot moltbot; do
    EXT_DIR="$HOME/.$APP_NAME/extensions/umibot"
    if [ -d "$EXT_DIR" ]; then
...
if [ -n "$SAVED_UMIBOT_TOKEN" ]; then
    echo ""
    echo "恢复 umibot 通道配置..."
    $CMD channels add --channel umibot --token "$SAVED_UMIBOT_TOKEN" 2>&1 || true
fi
...
if [ "$SAVED_MARKDOWN" = "true" ]; then
    ...
    if [ -f "$APP_CONFIG" ]; then
        node -e "
          ...
          cfg.channels.umibot.markdownSupport = true;
...
echo ""
echo "========================================="
echo "  ✅ Umi Bot 已从 ${LOCAL_VER:-未知} 更新到 ${NEW_VER}"
echo "========================================="
echo ""
echo "常用命令:"
