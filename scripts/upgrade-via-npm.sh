...
# 如果 channels.umibot 已存在但 umibot 插件尚未加载，校验会失败。
#
# ⚠️ 关键：绝不能直接修改真实的 openclaw.json，否则 gateway 的 config file watcher
#    会检测到变更并触发 SIGUSR1 重启，导致正在执行的升级脚本被杀死。
#
# 解决：创建临时配置副本（不含 channels.umibot），通过 OPENCLAW_CONFIG_PATH
...
if [ -f "$CONFIG_FILE" ]; then
    HAS_QQBOT_CHANNEL="$(node -e "
      try {
        ...
        if (cfg.channels && cfg.channels.umibot) process.stdout.write('true');
      } catch {}
    " 2>/dev/null || true)"
    ...
    if [ "$HAS_QQBOT_CHANNEL" = "true" ]; then
        ...
        " 2>/dev/null
        if [ $? -eq 0 ]; then
            echo "  [兼容] 创建临时配置副本（不含 channels.umibot）以通过配置校验"
...
if [ "$UPGRADE_OK" != "true" ]; then
    ...
    if [ -d "$EXTENSIONS_DIR/$PLUGIN_ID" ]; then
        BACKUP_DIR="$EXTENSIONS_DIR/.umibot-backup-$$"
        mv "$EXTENSIONS_DIR/$PLUGIN_ID" "$BACKUP_DIR"
        echo "  已备份旧目录: $BACKUP_DIR"
    fi

    # 清理历史遗留名称（这些不需要回滚）
    for dir_name in umibot openclaw-umi; do
    ...
    if $CMD plugins install "$INSTALL_SRC" --pin 2>&1; then
        ...
        find "$EXTENSIONS_DIR" -maxdepth 1 -name ".umibot-backup-*" -exec rm -rf {} + 2>/dev/null || true
    else
        echo "  ❌ install 失败"
        # 回滚：恢复旧目录
        if [ -n "$BACKUP_DIR" ] && [ -d "$BACKUP_DIR" ]; then
        ...
        echo "QQBOT_REPORT=❌ UMIBot 安装失败（已回滚到旧版本），请检查网络和 npm registry"
        exit 1
    fi
fi

# install/update 完成，恢复 channels.umibot
restore_umibot_channel
...
if [ "$PREFLIGHT_OK" != "true" ]; then
    ...
    echo "❌ 验证未通过"
    echo "QQBOT_NEW_VERSION=unknown"
    echo "QQBOT_REPORT=⚠️ UMIBot 升级异常，验证未通过"
    exit 1
fi
echo "  ✅ 验证全部通过"
...
if [ -n "$NEW_VERSION" ] && [ "$NEW_VERSION" != "unknown" ]; then
    ...
    echo "QQBOT_REPORT=⚠️ UMIBot 升级异常，无法确认新版本"
fi
...
if [ -n "$APPID" ] && [ -n "$SECRET" ]; then
    ...
    if [ "$CURRENT_TOKEN" = "$DESIRED_TOKEN" ]; then
        ...
        # umibot 是插件自定义通道，openclaw channels add --channel 不支持，
        # 直接编辑配置文件写入 channels.umibot
        CONFIG_FILE="$HOME/.$CMD/$CMD.json"
        if [ -f "$CONFIG_FILE" ] && node -e "
            ...
            cfg.channels.umibot.clientSecret = '$SECRET';
            fs.writeFileSync('$CONFIG_FILE', JSON.stringify(cfg, null, 4) + '\n');
        " 2>&1; then
            echo "  ✅ 通道配置写入成功"
        else
            echo "  ❌ 配置写入失败，请手动编辑 $CONFIG_FILE 添加 channels.umibot:"
            echo "     { \"channels\": { \"umibot\": { \"appId\": \"$APPID\", \"clientSecret\": \"...\" } } }"
        fi
    fi
...
if [ -n "$NEW_VERSION" ] && [ "$NEW_VERSION" != "unknown" ]; then
    MARKER_DIR="$HOME/.openclaw/umibot/data"
    mkdir -p "$MARKER_DIR"
...
if $CMD gateway restart 2>&1; then
    ...
    echo ""
    if [ -n "$NEW_VERSION" ] && [ "$NEW_VERSION" != "unknown" ]; then
        echo "🎉 UMIBot 插件已更新至 v${NEW_VERSION}，在线等候你的吩咐。"
    fi
else
