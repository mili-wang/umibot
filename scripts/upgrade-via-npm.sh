...
# umibot 通过 openclaw 原生插件指令升级（v4）
#
# 两级降级策略：
...
snapshot_config() {
    ...
    CONFIG_SNAPSHOT_FILE="$(mktemp "${TMPDIR:-/tmp}/.umibot-config-snapshot-XXXXXX")"
...
setup_temp_config() {
    ...
    need_temp="$(node -e "
      try {
        ...
        if (cfg.channels?.umibot || cfg.plugins?.allow?.includes('$PLUGIN_ID') || cfg.plugins?.entries?.['$PLUGIN_ID'])
    ...
    TEMP_CONFIG_FILE="$(mktemp "$OPENCLAW_HOME/.umibot-temp-config-XXXXXX")"
    if node -e "
      const fs = require('fs');
      const cfg = JSON.parse(fs.readFileSync('$CONFIG_FILE', 'utf8'));
...
npm_pack_download() {
    ...
    PACK_TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/.umibot-pack-XXXXXX")"
    PACK_TGZ_FILE=""
    local ok=false
    ensure_valid_cwd
...
npm_pack_native_install() {
    ...
    local extract_dir
    extract_dir="$(mktemp -d "${TMPDIR:-/tmp}/.umibot-extract-XXXXXX")"
...
cleanup_on_exit() {
    ...
    find "${TMPDIR:-/tmp}" -maxdepth 1 \( -name ".openclaw-install-stage-*" -o -name ".umibot-pack-*" \
        -o -name ".umibot-extract-*" -o -name ".umibot-upgrade-backup-*" \) -exec rm -rf {} + 2>/dev/null || true
    find "${OPENCLAW_HOME:-/dev/null}" -maxdepth 1 -name ".umibot-temp-config-*" -exec rm -f {} + 2>/dev/null || true
    release_upgrade_lock
    exit $exit_code
}
...
# 清理上次升级遗留（>60min）
find "${TMPDIR:-/tmp}" -maxdepth 1 \( -name ".umibot-upgrade-backup-*" -o -name ".umibot-pack-*" \
    -o -name ".umibot-extract-*" \) -mmin +60 -exec rm -rf {} + 2>/dev/null || true
find "${OPENCLAW_HOME:-/dev/null}" -maxdepth 1 -name ".umibot-temp-config-*" -mmin +60 -exec rm -f {} + 2>/dev/null || true

# ============================================================================
#  参数解析
# ============================================================================
PKG_NAME="@tencent-connect/openclaw-umibot"
PLUGIN_ID="openclaw-umibot"
TARGET_VERSION=""
APPID=""
SECRET=""
NO_RESTART=false
...
# 如果 OpenClaw 未来内置了 umibot 相关插件，在此列表中添加其 ID
BUILTIN_CONFLICT_IDS="umibot openclaw-qq"
...
# 默认禁用内置冲突插件（openclaw ≥2026.3.31 内置了 umibot 插件，与我们的 openclaw-umibot 冲突）
echo "[前置] 检查并禁用内置冲突插件..."
disable_builtin_plugins
echo ""
...
[ -f "$CONFIG_FILE" ] && node -e "
  try {
    ...
    const cfg = JSON.parse(fs.readFileSync('$CONFIG_FILE', 'utf8'));
    let c = false;
    for (const old of ['umibot', 'openclaw-qq']) {
...
if [ "$USE_UPDATE" = "true" ]; then
    ...
    if [ "$UPGRADE_OK" != "true" ]; then
        ...
        if [ -z "$BACKUP_DIR" ] && [ -d "$EXTENSIONS_DIR/$PLUGIN_ID" ]; then
            BACKUP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/.umibot-upgrade-backup-XXXXXX")"
...
if [ "$UPGRADE_OK" != "true" ]; then
    ...
    # 清理历史遗留
    for d in umibot openclaw-qq; do
    ...
    if [ $RC -eq 0 ] && [ -f "$EXTENSIONS_DIR/$PLUGIN_ID/package.json" ]; then
        ...
        if run_fallback; then
            ...
            echo "QQBOT_REPORT=❌ UMIBot 安装失败（已回滚），请检查网络"
            exit 1
        fi
    fi
fi
...
[ -n "$NEW_VERSION" ] && [ "$NEW_VERSION" != "unknown" ] && \
    echo "QQBOT_REPORT=✅ UMIBot 升级完成: v${NEW_VERSION}" || \
...
if [ -n "$APPID" ] && [ -n "$SECRET" ]; then
    echo ""
    echo "[配置] 写入 umibot 通道配置..."
    DESIRED="${APPID}:${SECRET}"
    CURRENT=""
    [ -f "$CONFIG_FILE" ] && CURRENT=$(node -e "
        try {
            const cfg = JSON.parse(require('fs').readFileSync('$CONFIG_FILE', 'utf8'));
            for (const k of ['umibot','openclaw-umibot','openclaw-qq']) {
    ...
    if [ "$CURRENT" = "$DESIRED" ]; then
        ...
        (cfg.channels ??= {}).umibot = { ...cfg.channels.umibot, appId: '$APPID', clientSecret: '$SECRET' };
...
if [ -n "$NEW_VERSION" ] && [ "$NEW_VERSION" != "unknown" ]; then
    MARKER_DIR="$OPENCLAW_HOME/umibot/data"; mkdir -p "$MARKER_DIR"
...
if [ $GW_RC -eq 0 ]; then
    ...
    [ -n "$NEW_VERSION" ] && echo "" && echo "🎉 UMIBot 插件已更新至 v${NEW_VERSION}，在线等候你的吩咐。"
else
    ...
    # 检测是否是 umibot 通道配置的 additional properties 校验错误
    if echo "$GW_OUTPUT" | grep -q "additional properties"; then
        echo ""
        log "config_fix" "start" "  [配置修复] 检测到 UMIBot 通道配置包含不支持的字段"
        ...
        node -e "
          try {
            ...
            cfg.channels.umibot = cleaned;
            fs.writeFileSync('$CONFIG_FILE', JSON.stringify(cfg, null, 4) + '\n');
            process.stdout.write('fixed');
          } catch (e) { process.stderr.write(String(e)); }
        " 2>/dev/null && log "config_fix" "success" "  [配置修复] ✅ 已清理 channels.umibot 中不支持的字段" || \
    ...
    ensure_valid_cwd
    _bak=""; [ -f "$CONFIG_FILE" ] && _bak="$(mktemp "${TMPDIR:-/tmp}/.umibot-pre-doctor-XXXXXX")" && cp -a "$CONFIG_FILE" "$_bak"
    ...
    if [ -n "$_bak" ] && [ -f "$_bak" ] && [ -f "$CONFIG_FILE" ]; then
        _damaged=$(node -e "
          try {
            ...
            if (b.channels?.umibot && !a.channels?.umibot) process.stdout.write('channels.umibot');
