  ...
  try {
    if (!configPath || !fs.existsSync(configPath)) {
      ...
      if (appId && clientSecret) {
        return { accountId: "default", appId: normalizeAppId(appId), umi6Sn: "", clientSecret, enabled: true, secretSource: "env", markdownSupport: true, config: {} };
      }
      return null;
    }
    const config = JSON.parse(fs.readFileSync(configPath, "utf-8"));
    const umibot = config.channels?.umibot;
    if (!umibot) return null;
    return {
      accountId: "default",
      ...
      secretSource: umibot.clientSecret ? "config" as const : "env" as const,
