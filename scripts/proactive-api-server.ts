...
import type { ResolvedUmiBotAccount } from "../src/types.js";

// 默认端口
const DEFAULT_PORT = 3721;

// 从配置文件加载账户信息
function loadAccount(accountId = "default"): ResolvedUmiBotAccount | null {
  const configPath = path.join(process.env.HOME || "/home/ubuntu", "clawd", "config.json");
  
  try {
    // 优先从环境变量获取
    const envAppId = process.env.UMIBOT_APP_ID;
    const envClientSecret = process.env.UMIBOT_CLIENT_SECRET;
    ...
    const umibot = config.channels?.umibot;
    ...
    if (accountId === "default") {
      return {
        accountId: "default",
        appId: umibot.appId || envAppId,
        clientSecret: umibot.clientSecret || envClientSecret,
        enabled: umibot.enabled ?? true,
        secretSource: umibot.clientSecret ? "config" : "env",
      };
    }
    
    const accountConfig = umibot.accounts?.[accountId];
    if (accountConfig) {
      return {
        accountId,
        appId: accountConfig.appId || umibot.appId || envAppId,
        clientSecret: accountConfig.clientSecret || umibot.clientSecret || envClientSecret,
...
async function handleRequest(req: http.IncomingMessage, res: http.ServerResponse) {
  ...
  try {
    ...
    if (pathname === "/" && method === "GET") {
      sendJson(res, 200, {
        name: "Umi Bot Proactive Message API",
        version: "1.0.0",
        endpoints: {
          "POST /send": {
            description: "发送主动消息",
...
function main() {
  ...
  server.listen(port, () => {
    console.log(`
╔═══════════════════════════════════════════════════════════════╗
║         Umi Bot Proactive Message API Server                    ║
╠═══════════════════════════════════════════════════════════════╣
