# umibot upgrade via npm package (Windows PowerShell)
#
...
if ($Help) {
    ...
    Write-Host ""
    Write-Host "  -Pkg [scope/name]    Custom npm package (e.g. ryantest/openclaw-umibot)"
    Write-Host "  -AppId [appid]       UMI bot appid (required on first install)"
    Write-Host "  -Secret [secret]     UMI bot secret (required on first install)"
    exit 0
}
...
Write-Host "==========================================="
Write-Host "  umibot npm upgrade: $INSTALL_SRC"
Write-Host "==========================================="
Write-Host ""

# [1/3] Download and extract new version
Write-Host "[1/5] Downloading new version..."
$TMPDIR_PACK = Join-Path ([System.IO.Path]::GetTempPath()) "umibot-pack-$([guid]::NewGuid().ToString('N').Substring(0,8))"
$EXTRACT_DIR = Join-Path ([System.IO.Path]::GetTempPath()) "umibot-extract-$([guid]::NewGuid().ToString('N').Substring(0,8))"
...
try {
    ...
    $STAGING_DIR = Join-Path (Split-Path $EXTENSIONS_DIR -Parent) ".umibot-upgrade-staging"
...
$TARGET_DIR = Join-Path $EXTENSIONS_DIR "umibot"
...
foreach ($leftover in @("umibot.staging", ".umibot-upgrade-staging", ".umibot-upgrade-old", ".umibot-new")) {
    $p = Join-Path $EXTENSIONS_DIR $leftover
    if (Test-Path $p) { Remove-Item -Recurse -Force $p -ErrorAction SilentlyContinue }
}
$oldDir = Join-Path (Split-Path $EXTENSIONS_DIR -Parent) ".umibot-upgrade-old"
if (Test-Path $oldDir) { Remove-Item -Recurse -Force $oldDir -ErrorAction SilentlyContinue }
foreach ($legacyName in @("umibot", "openclaw-umi")) {
...
if ($NEW_VERSION -ne "unknown") {
    Write-Host "QQBOT_REPORT=UMIBot upgrade complete: v${NEW_VERSION}"
} else {
    Write-Host "QQBOT_REPORT=[WARN] UMIBot upgrade status unknown, cannot confirm new version"
}
...
if ($AppId -and $Secret) {
    Write-Host ""
    Write-Host "[Config] Writing umibot channel config..."
    $DESIRED_TOKEN = "${AppId}:${Secret}"

    try {
        & $CMD channels add --channel umibot --token $DESIRED_TOKEN 2>&1 | Out-Null
        if ($LASTEXITCODE -eq 0) {
        ...
        if (Test-Path $CONFIG_FILE) {
            try {
                ...
                if (-not $cfg.channels.umibot) { $cfg.channels | Add-Member -NotePropertyName umibot -NotePropertyValue @{} }
                $cfg.channels.umibot | Add-Member -NotePropertyName appId -NotePropertyValue $AppId -Force
                $cfg.channels.umibot | Add-Member -NotePropertyName clientSecret -NotePropertyValue $Secret -Force
                ...
                Write-Host "     $CMD channels add --channel umibot --token `"${AppId}:${Secret}`""
            }
        }
    }
...
if ($NEW_VERSION -and $NEW_VERSION -ne "unknown") {
    $MarkerDir = Join-Path $HOME_DIR ".openclaw" "umibot" "data"
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        if ($NEW_VERSION -and $NEW_VERSION -ne "unknown") {
            Write-Host ""
            Write-Host "🎉 UMIBot 插件已更新至 v${NEW_VERSION}，在线等候你的吩咐。"
        }
