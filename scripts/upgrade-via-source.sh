...
if echo "$PROJ_DIR" | grep -q '/.openclaw/extensions\|/.clawdbot/extensions\|/.moltbot/extensions'; then
    ...
    echo ""
    echo "请从源码仓库运行:"
    echo "   cd /path/to/umibot && ./scripts/upgrade-via-source.sh"
    exit 1
fi
...
echo "========================================="
echo "  umibot 一键更新启动脚本"
echo "========================================="

# 1. 备份已有 umibot 通道配置，防止升级过程丢失
echo ""
echo "[1/6] 备份已有配置..."
SAVED_QQBOT_TOKEN=""
SAVED_QQBOT_CONFIG_FILE=""  # 有 umibot 配置的文件路径
SAVED_QQBOT_CHANNEL_JSON="" # 完整 channels.umibot JSON

# 完整备份 channels.umibot 对象（含 token / groups / env / allowFrom 等）
for APP_NAME in openclaw clawdbot moltbot; do
    ...
    if [ -f "$CONFIG_FILE" ]; then
        SAVED_QQBOT_CHANNEL_JSON=$(node -e "
            ...
            const keys = ['umibot', 'openclaw-umibot', 'openclaw-qq'];
...
if [ -z "$SAVED_QQBOT_CHANNEL_JSON" ] && [ -d "$HOME/.openclaw" ]; then
    SAVED_QQBOT_CHANNEL_JSON=$(node -e "
      ...
      for (const f of files) {
        try {
          ...
          const keys = ['umibot', 'umibot', 'openclaw-umi'];
          for (const key of keys) {
            const ch = cfg.channels && cfg.channels[key];
            if (ch && Object.keys(ch).length > 0) {
              process.stdout.write(JSON.stringify(ch));
    ...
    if [ -n "$SAVED_QQBOT_CHANNEL_JSON" ]; then
        ...
        " "$SAVED_QQBOT_CHANNEL_JSON" 2>/dev/null || true)
        echo "已从 ~/.openclaw/openclaw.json.bak* 恢复 umibot 通道配置"
        if [ -n "$SAVED_QQBOT_TOKEN" ]; then
            echo "  token: ${SAVED_QQBOT_TOKEN:0:10}..."
        fi
    fi
...
OLD_QQBOT_VERSION=$(node -e '
    try {
        const fs = require("fs");
        const path = require("path");
        const candidates = ["umibot", "umibot", "openclaw-umi"];
...
# ── 临时移除 channels.umibot 配置 ──
# openclaw CLI 任何子命令（包括 gateway stop、plugins install）启动时都会校验 openclaw.json，
# 如果 channels.umibot 存在但插件还未安装，CLI 不认识该 channel id，
# 导致 "Config invalid: unknown channel id: umibot" 而命令失败（鸡生蛋问题）。
# 解决方案：在所有 openclaw 命令之前把 channels.umibot 暂存，完成后恢复。
    ...
    if [ -f "$_cfg" ]; then
        _QQBOT_CHANNEL_STASH=$(node -e "
            ...
            if (cfg.channels && cfg.channels.umibot) {
                const stashed = JSON.stringify(cfg.channels.umibot);
        ...
        if [ -n "$_QQBOT_CHANNEL_STASH" ]; then
            _STASH_APP="$_app"
            _STASH_CFG="$_cfg"
            echo "  已暂存 channels.umibot 配置（避免 CLI 校验失败）"
        fi
        break
    fi
done
...
# 触发 restart，导致 "unknown channel id: umibot" 等错误
_gw_was_running=0
...
_INSTALL_DIR="$HOME/.openclaw/extensions/openclaw-umibot"
...
if [ ! -f "$_INSTALL_DIR/dist/index.js" ] || [ ! -f "$_INSTALL_DIR/preload.cjs" ]; then
        ...
        * )
            ...
            if [ -n "$_QQBOT_CHANNEL_STASH" ] && [ -n "$_STASH_CFG" ] && [ -f "$_STASH_CFG" ]; then
                _STASH="$_QQBOT_CHANNEL_STASH" _CFG="$_STASH_CFG" node -e '
                    ...
                    cfg.channels.umibot = JSON.parse(process.env._STASH);
    ...
    for _app in openclaw clawdbot moltbot; do
        ...
        if [ -f "$_cfg" ]; then
            node -e "
              const fs = require('fs');
              const cfg = JSON.parse(fs.readFileSync('$_cfg', 'utf8'));
              const inst = cfg.plugins && cfg.plugins.installs && cfg.plugins.installs['umibot'];
              if (inst && inst.source !== 'path') {
                inst.source = 'path';
                inst.sourcePath = '$PROJ_DIR';
                fs.writeFileSync('$_cfg', JSON.stringify(cfg, null, 4) + '\n');
                console.log('  已将 plugins.installs.umibot.source 更新为 path');
              }
            " 2>/dev/null || true
            break
        fi
    ...
    _plugin_dir_ok=0
    for _candidate_name in umibot umibot openclaw-umi; do
        ...
        if [ "$_plugin_dir_ok" -eq 0 ]; then
            ...
            node -e "
              ...
              for (const app of ['openclaw', 'clawdbot', 'moltbot']) {
                ...
                const cfg = JSON.parse(fs.readFileSync(f, 'utf8'));
                let changed = false;
                // 移除 channels.umibot（插件未加载时此配置会导致 unknown channel id 错误）
                if (cfg.channels && cfg.channels.umibot) { delete cfg.channels.umibot; changed = true; }
                // 清理 plugins.allow 中的 umibot
                if (cfg.plugins && Array.isArray(cfg.plugins.allow)) {
                  cfg.plugins.allow = cfg.plugins.allow.filter(x => x !== 'umibot');
                  changed = true;
                }
            ...
            " 2>/dev/null || true
            echo "  已清理无效配置，gateway 可正常启动（无 umibot 插件状态）"
    ...
    PLUGIN_NM=""
    for _candidate in umibot umibot openclaw-umi; do
    ...
    find "$HOME/.openclaw/extensions/" -maxdepth 1 -name ".openclaw-umibot-backup-*" -exec rm -rf {} + 2>/dev/null || true
    find "${TMPDIR:-/tmp}" -maxdepth 1 -name ".umibot-upgrade-backup-*" -exec rm -rf {} + 2>/dev/null || true

    # 恢复 channels.umibot 完整配置（防止 plugins install 意外覆盖）
    ...
    if [ -n "$SAVED_QQBOT_CHANNEL_JSON" ]; then
        node -e "
          ...
          for (const app of ['openclaw', 'clawdbot', 'moltbot']) {
            ...
            const cfg = JSON.parse(fs.readFileSync(f, 'utf8'));
            if (!cfg.channels) { cfg.channels = {}; }
            if (JSON.stringify(cfg.channels.umibot) === JSON.stringify(saved)) {
              process.stderr.write('  channels.umibot 配置无变化，无需恢复\\n');
            } else {
              cfg.channels.umibot = saved;
              fs.writeFileSync(f, JSON.stringify(cfg, null, 4) + '\\n');
              process.stderr.write('  已完整恢复 channels.umibot 配置（' + Object.keys(saved).join(', ') + '）\\n');
            }
            break;
          }
...
# ── 暂不恢复 channels.umibot 配置 ──
# openclaw 3.23+ 启动时在插件加载前就校验 channels，
# 如果此时恢复 channels.umibot，gateway 会因 "unknown channel id: umibot" 拒绝启动。
# 延迟到 gateway 启动、插件加载完成后再恢复（见 Step 6）。
echo "  [兼容] channels.umibot 将在 gateway 启动后恢复（避免启动校验失败）"

# ── Step 4/5：不在此时写入 channels.umibot 配置 ──
# openclaw 3.23+ 在插件加载前校验 channels，此时写入 channels.umibot 会导致
# "unknown channel id: umibot" 错误。所有 channels.umibot 相关配置延迟到
...
# 读取当前 umibot token（从暂存或配置文件）
# 注意：channels.umibot 已被暂存移除，所以从 _QQBOT_CHANNEL_STASH 读取
CURRENT_QQBOT_TOKEN=""
if [ -n "$_QQBOT_CHANNEL_STASH" ]; then
...
if [ -n "$DESIRED_QQBOT_TOKEN" ]; then
    ...
    echo "  [兼容] 将在 gateway 启动后写入 channels.umibot"
elif [ -z "$CURRENT_QQBOT_TOKEN" ] && [ -z "$_QQBOT_CHANNEL_STASH" ]; then
    echo ""
    echo "❌ 未检测到 umibot 通道配置！"
    echo ""
    ...
    echo ""
    echo "appid 和 appsecret 可在 QQ 开放平台 (https://q.umi.com) 获取。"
    exit 1
else
    echo "使用已有配置（暂存中）"
fi
...
if [ "$OLD_QQBOT_VERSION" = "$NEW_QQBOT_VERSION" ]; then
    echo "umibot 插件版本: $NEW_QQBOT_VERSION (未变化)"
elif [ "$OLD_QQBOT_VERSION" = "not_installed" ]; then
    echo "umibot 插件版本: $NEW_QQBOT_VERSION (新安装)"
else
    echo "umibot 插件版本: $OLD_QQBOT_VERSION -> $NEW_QQBOT_VERSION"
fi
echo ""
...
case "$start_choice" in
    y|yes)
        ...
        # 端口就绪后：恢复 channels.umibot 配置 + 检查连接 + 跟踪日志
        if [ "$_port_ready" -eq 1 ]; then
            echo "✅ Gateway 端口已就绪（插件已加载）"
            echo ""

            # ── 恢复 channels.umibot 配置 ──
            # gateway 已启动、插件已注册 umibot channel，现在可以安全写回配置
            _need_reload=0
            _target_cfg=""
            for _app in openclaw clawdbot moltbot; do
                _cfg="$HOME/.$_app/$_app.json"
            ...
            if [ -n "$_target_cfg" ]; then
                # 构建完整的 channels.umibot 对象（合并暂存配置 + 新 token + markdown）
                ...
                node -e '
                    ...
                    if (stash) {
                        try { cfg.channels.umibot = JSON.parse(stash); } catch {}
                    }
                    if (!cfg.channels.umibot) cfg.channels.umibot = {};

                    // 覆盖 token（如果有新值）
                    const desired = process.env._DESIRED;
                    if (desired && desired.includes(":")) {
                        const [appId, ...rest] = desired.split(":");
                        cfg.channels.umibot.appId = appId;
                        cfg.channels.umibot.clientSecret = rest.join(":");
                        delete cfg.channels.umibot.token;
                    }

                    // 覆盖 markdown（如果有指定）
                    const md = process.env._MD;
                    if (md === "true") cfg.channels.umibot.markdownSupport = true;
                    else if (md === "false") cfg.channels.umibot.markdownSupport = false;

                    fs.writeFileSync(process.env._CFG, JSON.stringify(cfg, null, 4) + "\n");
                ' 2>&1 || echo "  ⚠️  配置写入失败"
                echo "  ✅ 已恢复 channels.umibot 配置（含 token/markdown）"
                _need_reload=1
            fi
            ...
            # 检查 umibot WS 是否连接成功（最多等 20 秒）
            echo "检查 umibot 插件连接状态..."
            ...
            for _j in $(seq 1 10); do
                ...
                printf "\r  等待 umibot WS 连接... (%d/10)" "$_j"
                sleep 2
            done
            echo ""

            if [ "$_umibot_ready" -eq 0 ]; then
                echo "⚠️  umibot 插件可能未正确加载"
    ...
    n|no)
        echo ""
        echo "✅ 插件更新完毕，未启动服务"
        # 不启动时也需要恢复 channels.umibot，否则配置丢失
        # 注意：下次 gateway 启动可能因 "unknown channel id" 失败，
        # 需要用户手动 stop → 移除 channels.umibot → start → 恢复
        if [ -n "$_QQBOT_CHANNEL_STASH" ] && [ -n "$_STASH_CFG" ] && [ -f "$_STASH_CFG" ]; then
            _STASH="$_QQBOT_CHANNEL_STASH" _CFG="$_STASH_CFG" node -e '
                const fs = require("fs");
                const cfg = JSON.parse(fs.readFileSync(process.env._CFG, "utf8"));
                ...
                fs.writeFileSync(process.env._CFG, JSON.stringify(cfg, null, 4) + "\n");
            ' 2>/dev/null || true
            echo "  已恢复 channels.umibot 配置"
        fi
        echo ""
        echo "后续启动方法（兼容 openclaw 3.23+）："
        echo "  1. 先临时移除 channels.umibot 再启动："
