...
set "extensionDir=%appDir%\extensions\umibot"
...
if exist "%configFile%" (
    echo Cleaning umibot fields from config...
    set "configPath=%configFile:\=/%"
    node -e "const fs=require('fs');const c=JSON.parse(fs.readFileSync('!configPath!','utf8'));if(c.channels&&c.channels.umibot){delete c.channels.umibot;console.log('  - deleted channels.umibot');}if(c.plugins&&c.plugins.entries&&c.plugins.entries.umibot){delete c.plugins.entries.umibot;console.log('  - deleted plugins.entries.umibot');}if(c.plugins&&c.plugins.installs&&c.plugins.installs.umibot){delete c.plugins.installs.umibot;console.log('  - deleted plugins.installs.umibot');}if(c.plugins&&c.plugins.allow&&Array.isArray(c.plugins.allow)){const i=c.plugins.allow.indexOf('umibot');if(i^!==-1){c.plugins.allow.splice(i,1);console.log('  - deleted umibot from plugins.allow array');}}fs.writeFileSync('!configPath!',JSON.stringify(c,null,2));console.log('Config file updated');" || (
