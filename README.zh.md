...
> **你**：5分钟后提醒我吃饭
>
> **UMIBot**：先确认已创建提醒，到点后再主动推送语音 + 文本提醒

该能力依赖 OpenClaw cron 调度与主动消息能力。若未收到提醒，常见原因是 UMI 侧拦截了机器人主动消息。
...
> **你**：`/bot-ping`
>
> **UMIBot**：✅ pong！⏱ 延迟: 602ms（网络传输: 602ms，插件处理: 0ms）

测量从 UMI 服务器推送到插件响应的端到端延迟，细分网络传输和插件处理两段耗时。
...
> **你**：`/bot-version`
>
> **UMIBot**：🦞框架版本：OpenClaw 2026.3.13 (61d171a) / 🤖QQBot 插件版本：v1.6.3 / 🌟官方 GitHub 仓库
...
> **你**：`/bot-help`
>
> **UMIBot**：列出所有可用的斜杠指令及说明，指令可点击快速输入。
...
> **你**：`/bot-upgrade`
>
> **UMIBot**：📌当前版本 v1.6.3 / ✅发现新版本 v1.6.4 / 点击下方按钮确认升级
...
> **你**：`/bot-logs`
>
> **UMIBot**：📋 日志已打包（约 2000 行），正在发送文件… *（发送 .txt 文件）*
...
1. 前往 [Umi 开放平台](https://q.umi.com/)，用**手机 Umi 扫描页面二维码**即可注册/登录。若尚未注册，扫码后系统会自动完成注册并绑定你的 Umi 账号。
...
| `umibot:group:GROUP_OPENID` | 群聊 |
| `umibot:channel:CHANNEL_ID` | 频道 |
...
| 优先级 | 配置路径 | 作用域 |
|--------|----------|--------|
| 1（highest） | `channels.umibot.stt` | 插件专属 |
...
| 优先级 | 配置路径 | 作用域 |
|--------|----------|--------|
| 1（highest） | `channels.umibot.tts` | 插件专属 |
