...
const PKG_NAME = "@mili-wang/umibot";
...
async function fetchDistTags(): Promise<Record<string, string>> {
  for (const url of REGISTRIES) {
    try {
      ...
      _log?.debug?.(`[umibot:update-checker] ${url} failed: ${e.message}`);
    }
  }
...
export function triggerUpdateCheck(log?: {
  ...
  getUpdateInfo().then((info) => {
    if (info.hasUpdate) {
      _log?.info?.(`[umibot:update-checker] new version available: ${info.latest} (current: ${CURRENT_VERSION})`);
    }
  }).catch(() => {});
}
...
export async function getUpdateInfo(): Promise<UpdateInfo> {
  try {
    ...
    _log?.debug?.(`[umibot:update-checker] check failed: ${err.message}`);
 ...
 * @param pkgName 可选的包名（如 "@ryantest/openclaw-umibot"），默认使用内置包名
 */
