...
export interface MessageTarget {
  ...
  channelId?: string;
  groupOpenid?: string;
  /** C2C 入站事件携带的 room_id，被动回复 lobster API 需要 */
  room_id?: string;
}
...
export async function sendWithTokenRetry<T>(
  ...
  try {
    ...
    if (errMsg.includes("401") || errMsg.includes("token") || errMsg.includes("access_token")) {
      log?.info(`[umibot:${accountId}] Token may be expired, refreshing...`);
      clearTokenCache(appId);
      const newToken = await getAccessToken(appId, clientSecret, umi6Sn);
...
export async function sendTextToTarget(
  ...
  await sendWithTokenRetry(account.appId, account.clientSecret, account.umi6Sn, async (token) => {
    if (target.type === "c2c") {
      await sendC2CMessage(token, target.senderId, text, account.umi6Sn, target.messageId, target.room_id, refIdx);
...
export async function sendErrorToTarget(ctx: ReplyContext, errorText: string): Promise<void> {
  try {
    ...
    ctx.log?.error(`[umibot:${ctx.account.accountId}] Failed to send error message: ${sendErr}`);
  }
}
  ...
  if (payload.source === "file") {
    try {
      if (!(await fileExistsAsync(imageUrl))) {
        log?.error(`[umibot:${account.accountId}] Image not found: ${imageUrl}`);
        return;
      }
      const imgSzCheck = checkFileSize(imageUrl, getMaxUploadSize(1)); // IMAGE = 1
      if (!imgSzCheck.ok) {
        log?.error(`[umibot:${account.accountId}] Image size check failed: ${imgSzCheck.error}`);
        return;
      }
...
async function handleAudioPayload(ctx: ReplyContext, payload: MediaPayload): Promise<void> {
  ...
  try {
    ...
    if (!ttsText?.trim()) {
      log?.error(`[umibot:${account.accountId}] Voice missing text`);
    } else {
      ...
      if (!ttsCfg) {
        ...
        log?.info(`[umibot:${account.accountId}] TTS done: ${formatDuration(duration)}, file saved: ${silkPath}`);
        ...
        await sendWithTokenRetry(account.appId, account.clientSecret, account.umi6Sn, async (token) => {
          if (target.type === "c2c") {
            ...
            log?.error(`[umibot:${account.accountId}] Voice not supported in channel, sending text fallback`);
...
async function handleVideoPayload(ctx: ReplyContext, payload: MediaPayload): Promise<void> {
  ...
  try {
    ...
    if (!videoPath?.trim()) {
      log?.error(`[umibot:${account.accountId}] Video missing path`);
    } else {
      ...
      await sendWithTokenRetry(account.appId, account.clientSecret, account.umi6Sn, async (token) => {
        if (isHttpUrl) {
          if (target.type === "c2c") {
            ...
            log?.error(`[umibot:${account.accountId}] Video not supported in channel`);
          }
        } else {
...
async function handleFilePayload(ctx: ReplyContext, payload: MediaPayload): Promise<void> {
  ...
  try {
    ...
    if (!filePath?.trim()) {
      log?.error(`[umibot:${account.accountId}] File missing path`);
    } else {
      ...
      await sendWithTokenRetry(account.appId, account.clientSecret, account.umi6Sn, async (token) => {
        if (isHttpUrl) {
          if (target.type === "c2c") {
            ...
            log?.error(`[umibot:${account.accountId}] File not supported in channel`);
          }
        } else {
