...
/**
 * /bot-ping — 测试当前 openclaw 与 UMI 连接的网络延迟
 */
registerCommand({
  name: "bot-ping",
  description: "测试当前 openclaw 与 UMI 连接的网络延迟",
  usage: [
    `/bot-ping`,
    ``,
    `测试 OpenClaw 主机与 UMI 服务器之间的网络延迟。`,
    `返回网络传输耗时和插件处理耗时。`,
  ].join("\n"),
  handler: (ctx) => {
    const now = Date.now();
    ...
    const umiToPlugin = ctx.receivedAt - eventTime;
...
registerCommand({
  ...
  usage: [
    `/bot-version`,
    ``,
    `查看当前 UMIBot 插件版本和 OpenClaw 框架版本。`,
    `同时检查是否有新版本可用。`,
  ].join("\n"),
  handler: async () => {
    ...
    if (info.checkedAt === 0) {
      ...
      lines.push(`🆕最新可用版本：v${info.latest}，点击 <umibot-cmd-input text="/bot-upgrade" show="/bot-upgrade"/> 查看升级指引`);
    } 
    lines.push(`🌟官方 GitHub 仓库：[点击前往](https://github.com/mili-wang/umibot/)`);
    return lines.join("\n");
  },
});
...
registerCommand({
  ...
  handler: (ctx) => {
    ...
    for (const [name, cmd] of commands) {
      ...
      lines.push(`<umibot-cmd-input text="/${name}" show="/${name}"/> ${cmd.description}`);
    }
...
const DEFAULT_UPGRADE_URL = "https://docs.umi.com/doc/DSGxOZk1oVnVKVkpq";
...
function findCli(): string | null {
  ...
  // 典型路径: .../gateway/node_modules/umibot/dist/src/slash-commands.js
...
function getUpgradeScriptPath(): string | null {
  ...
  const candidates = [
    ...
    // npm 安装: node_modules/@mili-wang/umibot/dist/src → ../../scripts
    path.resolve(currentDir, "..", "scripts", scriptName),
  ...
  const homeDir = getHomeDir();
  for (const cli of ["openclaw", "clawdbot", "moltbot"]) {
    candidates.push(path.join(homeDir, `.${cli}`, "extensions", "umibot", "scripts", scriptName));
  }
...
/**
 * 将 openclaw.json 中的 umibot 插件 source 从 "path" 切换为 "npm"。
 * 用于热更新场景：从 npm 拉取新版本后，确保 openclaw 不再从本地源码加载。
 *
 * 安全保障：写回配置前验证 channels.umibot 未丢失，防止竞态写入导致凭证消失。
 */
function switchPluginSourceToNpm(): void {
  try {
    ...
    for (const cli of ["openclaw", "clawdbot", "moltbot"]) {
      ...
      const inst = cfg?.plugins?.installs?.["umibot"];
      ...
      const tmpPath = cfgPath + ".umibot-upgrade.tmp";
...
function preUpgradeCredentialBackup(accountId: string, appId: string): void {
  try {
    ...
    for (const cli of ["openclaw", "clawdbot", "moltbot"]) {
      ...
      let secret = "";
      if (accountId === "default" && umibot.clientSecret) {
        secret = umibot.clientSecret;
      } else if (umibot.accounts?.[accountId]?.clientSecret) {
        secret = umibot.accounts[accountId].clientSecret;
      } else if (umibot.clientSecret) {
        secret = umibot.clientSecret;
      }
      if (appId && secret) {
...
function copyScriptToTemp(scriptPath: string): string | null {
  try {
    ...
    const tmpDir = path.join(getHomeDir(), ".openclaw", ".umibot-upgrade-tmp");
...
const REMOTE_UPGRADE_SCRIPT_URL = "https://raw.githubusercontent.com/tencent-connect/umibot/main/scripts/upgrade-via-npm.sh";
const REMOTE_UPGRADE_SCRIPT_URL_WIN = "https://raw.githubusercontent.com/tencent-connect/umibot/main/scripts/upgrade-via-npm.ps1";
...
function downloadRemoteUpgradeScript(): string | null {
  try {
    ...
    if (!fs.existsSync(tmpScript) || fs.statSync(tmpScript).size < 100) {
      console.error(`[umibot] downloadRemoteUpgradeScript: downloaded file too small or missing`);
      return null;
    }
    ...
    console.log(`[umibot] downloadRemoteUpgradeScript: fetched from ${url} → ${tmpScript}`);
    return tmpScript;
  } catch (e: any) {
    console.error(`[umibot] downloadRemoteUpgradeScript: failed: ${e.message}`);
    return null;
  }
}
...
function fireHotUpgrade(targetVersion?: string): HotUpgradeStartResult {
  ...
  execFile(shell, shellArgs, {
    ...
    console.log(`[umibot] fireHotUpgrade: script completed, stdout length=${stdout.length}`);
    ...
    if (isWindows()) {
      ...
      const cliInvoke = cli.endsWith(".mjs")
        ...
        : `& '${cli}'`;
      const ps1Content = [
        `Write-Host '[umibot-upgrade] Stopping gateway...'`,
        `${cliInvoke} gateway stop`,
        `Write-Host '[umibot-upgrade] Waiting for process to exit...'`,
        `Start-Sleep -Seconds 3`,
        `Write-Host '[umibot-upgrade] Starting gateway...'`,
        `${cliInvoke} gateway start`,
        `Write-Host '[umibot-upgrade] Done.'`,
        `Remove-Item -LiteralPath $MyInvocation.MyCommand.Path -Force -ErrorAction SilentlyContinue`,
      ].join("\r\n");
      const ps1Path = path.join(getHomeDir(), ".openclaw", ".umibot-restart.ps1");
      const ps = findPowerShell();
      try {
        ...
        child.unref();
        console.log(`[umibot] fireHotUpgrade: launched detached restart script (pid=${child.pid}): ${ps1Path}`);
      } catch (psErr: any) {
        console.error(`[umibot] fireHotUpgrade: failed to launch ps1 restart: ${psErr.message}, falling back to direct restart`);
...
registerCommand({
  ...
  handler: async (ctx) => {
    ...
    const GITHUB_URL = "https://github.com/tencent-connect/umibot/";
    ...
    if (!versionArg && !isLatest && !isForce) {
      ...
      return [
        ...
        `请确认主机网络可正常访问 npm 仓库。`,
        ``,
        `**点击确认升级** <umibot-cmd-enter text="/bot-upgrade --latest" />`,
        ``,
        `手动升级指引：[点击查看](${url})`,
        `🌟官方 GitHub 仓库：[点击前往](${GITHUB_URL})`,
      ].join("\n");
...
registerCommand({
  ...
  handler: (ctx) => {
    ...
    // 路径: downloads/{appId}/
    const targetDir = path.join(getHomeDir(), ".openclaw", "media", "umibot", "downloads", appId);
    const displayDir = `~/.openclaw/media/umibot/downloads/${appId}`;
    ...
    if (!isForce) {
      ...
      lines.push(
        ...
        `‼️ 点击指令确认删除`,
        `<umibot-cmd-enter text="/bot-clear-storage --force" />`,
      );
