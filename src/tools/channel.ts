...
const API_BASE = "https://api.sgroup.umi.com";
 ...
 * 注册 UMI 频道 API 代理工具。
 *
 * 该工具作为 UMI 开放平台频道 API 的 HTTP 代理，自动处理 Token 鉴权。
...
export function registerChannelTool(api: OpenClawPluginApi): void {
  ...
  if (!cfg) {
    console.log("[umibot-channel-api] No config available, skipping");
    return;
  }

  const accountIds = listQQBotAccountIds(cfg);
  if (accountIds.length === 0) {
    console.log("[umibot-channel-api] No UMIBot accounts configured, skipping");
    return;
  }
  ...
  if (!account.appId || !account.clientSecret) {
    console.log("[umibot-channel-api] Account not fully configured, skipping");
    return;
  }

  api.registerTool(
    {
      name: "umibot_channel_api",
      label: "UMIBot Channel API",
      description:
        "UMI 开放平台频道 API HTTP 代理，自动填充鉴权 Token。" +
        "常用接口速查：" +
        ...
        "更多接口和参数详情请阅读 umibot-channel skill。",
      ...
      async execute(_toolCallId, params) {
        ...
        if ((method === "GET" || method === "DELETE") && p.body && Object.keys(p.body).length > 0) {
          console.log(`[umibot-channel-api] ${method} request with body, body will be ignored`);
        }
        ...
        try {
          ...
          console.log(`[umibot-channel-api] >>> ${method} ${url} (timeout: ${DEFAULT_TIMEOUT_MS}ms)`);
          ...
          try {
            ...
            if (err instanceof Error && err.name === "AbortError") {
              console.error(`[umibot-channel-api] <<< Request timeout after ${DEFAULT_TIMEOUT_MS}ms`);
              return json({ error: `请求超时（${DEFAULT_TIMEOUT_MS}ms）`, path: p.path });
            }
            console.error(`[umibot-channel-api] <<< Network error:`, err);
            return json({
          ...
          console.log(`[umibot-channel-api] <<< Status: ${res.status} ${res.statusText}`);
          ...
          console.error(`[umibot-channel-api] Error [${method} ${p.path}]: ${errMsg}`);
  ...
  console.log("[umibot-channel-api] Registered UMI channel API proxy tool");
}
