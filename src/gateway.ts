...
async function ensureImageServer(log?: GatewayContext["log"], publicBaseUrl?: string): Promise<string | null> {
  ...
  try {
    ...
    log?.info(`[umibot] Image server started on port ${IMAGE_SERVER_PORT}, baseUrl: ${config.baseUrl}`);
    return config.baseUrl!;
  } catch (err) {
    log?.error(`[umibot] Failed to start image server: ${err}`);
    return null;
  }
}
...
export async function startGateway(ctx: GatewayContext): Promise<void> {
  ...
  if (diag.warnings.length > 0) {
    for (const w of diag.warnings) {
      log?.info(`[umibot:${account.accountId}] ${w}`);
    }
  }
  ...
  if (ttsCfg) {
    const maskedKey = ttsCfg.apiKey.length > 8
      ...
      : "****";
    log?.info(`[umibot:${account.accountId}] TTS configured: model=${ttsCfg.model}, voice=${ttsCfg.voice}, authStyle=${ttsCfg.authStyle ?? "bearer"}, baseUrl=${ttsCfg.baseUrl}`);
    log?.info(`[umibot:${account.accountId}] TTS apiKey: ${maskedKey}${ttsCfg.queryParams ? `, queryParams=${JSON.stringify(ttsCfg.queryParams)}` : ""}${ttsCfg.speed !== undefined ? `, speed=${ttsCfg.speed}` : ""}`);
  } else {
    log?.info(`[umibot:${account.accountId}] TTS not configured (voice messages will be unavailable)`);
  }
  ...
  if (account.imageServerBaseUrl) {
    ...
    log?.info(`[umibot:${account.accountId}] Image server enabled with URL: ${imageServerBaseUrl}`);
  } else {
  ...
  const scheduleReconnect = (customDelay?: number) => {
    if (isAborted || reconnectAttempts >= MAX_RECONNECT_ATTEMPTS) {
      log?.error(`[umibot:${account.accountId}] Max reconnect attempts reached or aborted`);
      return;
    }
    ...
    reconnectAttempts++;
    log?.info(`[umibot:${account.accountId}] Reconnecting in ${delay}ms (attempt ${reconnectAttempts})`);
  ...
  const connect = async () => {
    // 防止并发连接
    if (isConnecting) {
      log?.debug?.(`[umibot:${account.accountId}] Already connecting, skip`);
      return;
    }
    isConnecting = true;
    ...
    try {
      ...
      if (shouldRefreshToken) {
        log?.info(`[umibot:${account.accountId}] Refreshing token...`);
      ...
      const handleMessage = async (event: {
        ...
        log?.info(`[umibot:${account.accountId}] Processing message from ${event.senderId}: ${event.content}`);
        if (event.attachments?.length) {
          log?.info(`[umibot:${account.accountId}] Attachments: ${event.attachments.length}`);
        }
...
`;
      ...
      ws.on("open", () => {
        log?.info(`[umibot:${account.accountId}] WebSocket connected`);
        ...
        try {
          ...
          switch (op) {
            ...
            case 11: // Heartbeat ACK
              log?.debug?.(`[umibot:${account.accountId}] Heartbeat ACK`);
              break;

            case 7: // Reconnect
              log?.info(`[umibot:${account.accountId}] Server requested reconnect`);
              cleanup();
              scheduleReconnect();
              break;
          ...
          log?.error(`[umibot:${account.accountId}] Message parse error: ${err}`);
        }
      });
      ...
      ws.on("close", (code, reason) => {
        ...
        if (code === 4004) {
          log?.info(`[umibot:${account.accountId}] Invalid token (4004), will refresh token and reconnect`);
          shouldRefreshToken = true;
          cleanup();
          if (!isAborted) {
            scheduleReconnect();
        ...
        if (code === 4008) {
          log?.info(`[umibot:${account.accountId}] Rate limited (4008), waiting ${RATE_LIMIT_DELAY}ms before reconnect`);
          cleanup();
          if (!isAborted) {
        ...
        if (code === 4006 || code === 4007 || code === 4009) {
          ...
          log?.info(`[umibot:${account.accountId}] Error ${code} (${codeDesc[code]}), will re-identify`);
          sessionId = null;
          lastSeq = null;
          // 清除持久化的 Session
          clearSession(account.accountId);
          ...
          log?.info(`[umibot:${account.accountId}] Internal error (${code}), will re-identify`);
          sessionId = null;
          lastSeq = null;
          // 清除持久化的 Session
          clearSession(account.accountId);
        ...
        if (connectionDuration < QUICK_DISCONNECT_THRESHOLD && lastConnectTime > 0) {
          ...
          log?.info(`[umibot:${account.accountId}] Quick disconnect detected (${connectionDuration}ms), count: ${quickDisconnectCount}`);
      ...
      ws.on("error", (err) => {
        log?.error(`[umibot:${account.accountId}] WebSocket error: ${err.message}`);
        onError?.(err);
      });
      ...
      isConnecting = false; // 释放锁
      const errMsg = String(err);
      log?.error(`[umibot:${account.accountId}] Connection failed: ${err}`);
      
      // 如果是频率限制错误，等待更长时间
      if (errMsg.includes("Too many requests") || errMsg.includes("100001")) {
        log?.info(`[umibot:${account.accountId}] Rate limited, waiting ${RATE_LIMIT_DELAY}ms before retry`);
        scheduleReconnect(RATE_LIMIT_DELAY);
      } else {
        scheduleReconnect();
      }
