...
export interface QQBotApprovalHandlerOpts {
  ...
  appId: string;
  clientSecret: string;
  /** 租户路径段，与 getAccessToken / lobster.Content 一致 */
  umi6Sn: string;
  cfg: OpenClawConfig;
  gatewayUrl?: string;
  log?: {
...
function resolveTarget(
  ...
  // 优先从 sessionKey 解析（如 agent:main:umibot:direct:OPENID）
  const sk = sessionKey ?? turnSourceTo;
  if (!sk) return null;
  const m = sk.match(/umibot:(c2c|direct|group):([A-F0-9]+)/i);
  if (!m) return null;
...
export class QQBotApprovalHandler {
  ...
  async start(): Promise<void> {
    ...
    this.started = true;
    const { log } = this.opts;
    log?.info(`[umibot:${this.opts.accountId}] approval-handler: starting`);
    ...
    try {
      ...
      log?.error(`[umibot:${this.opts.accountId}] approval-handler: gateway-runtime module not available, approval feature disabled. Error: ${err}`);
      this.started = false;
      return;
    }
    ...
    try {
      this.gatewayClient = await gatewayRuntime.createOperatorApprovalsGatewayClient({
        ...
        clientDisplayName: "UMIBot Approval Handler",
        onEvent: (evt: EventFrame) => this.handleGatewayEvent(evt),
        onHelloOk: () => log?.info(`[umibot:${this.opts.accountId}] approval-handler: connected to gateway`),
        onConnectError: (err: { message: string }) => log?.error(`[umibot:${this.opts.accountId}] approval-handler: connect error: ${err.message}`),
        onClose: (code: number, reason: string) => log?.debug?.(`[umibot:${this.opts.accountId}] approval-handler: gateway closed: ${code} ${reason}`),
      });
      this.gatewayClient.start();
      setApprovalFeatureAvailable(true);
    } catch (err) {
      log?.error(`[umibot:${this.opts.accountId}] approval-handler: failed to create gateway client: ${err}`);
      this.started = false;
    }
  }
  ...
  async stop(): Promise<void> {
    ...
    this.opts.log?.info(`[umibot:${this.opts.accountId}] approval-handler: stopped`);
  }
  ...
  async resolveApproval(
    ...
    this.opts.log?.info(`[umibot:${this.opts.accountId}] approval-handler: resolving ${fullId} (input=${approvalId}) kind=${kind} → ${decision}, pending=${isPending}, cached=${isCached}`);

    try {
      await this.gatewayClient.request(method, { id: fullId, decision });
      this.opts.log?.info(`[umibot:${this.opts.accountId}] approval-handler: RPC success ${toShortId(fullId)} → ${decision} (method=${method})`);
      return true;
    } catch (err) {
      this.opts.log?.error(`[umibot:${this.opts.accountId}] approval-handler: resolve failed: ${err}`);
      return false;
    }
  }
  ...
  private async handleRequested(
    ...
    const { log, appId, clientSecret, accountId, umi6Sn } = this.opts;
    ...
    if (!target) {
      log?.info(`[umibot:${accountId}] approval-handler: no QQ target for ${shortId} (session=${sessionKey})`);
      return;
    }
    ...
    log?.info(`[umibot:${accountId}] approval-handler: sending ${kind} approval ${shortId} to ${target.type}:${target.id}`);
    ...
    try {
      const token = await getAccessToken(appId, clientSecret, umi6Sn);
      if (target.type === "c2c") {
        await sendC2CMessageWithInlineKeyboard(token, target.id, text, umi6Sn, keyboard);
      } else {
        await sendGroupMessageWithInlineKeyboard(token, target.id, text, keyboard);
      }
      log?.info(`[umibot:${accountId}] approval-handler: sent ${kind} approval ${shortId}`);
      ...
      log?.error(`[umibot:${accountId}] approval-handler: failed to send approval ${shortId}: ${err}`);
    }
  }
  ...
  private async handleResolved(
    ...
    this.opts.log?.info(
      `[umibot:${this.opts.accountId}] approval-handler: gateway confirmed ${toShortId(resolved.id)} → ${resolved.decision} (kind=${kind}, resolvedBy=${resolvedBy}, wasPending=${!!entry})`
    );
  ...
  private async handleTimeout(
    ...
    log?.info(`[umibot:${accountId}] approval-handler: timeout ${toShortId(approvalId)}`);
