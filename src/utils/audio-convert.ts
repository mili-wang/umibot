...
import { hasSilkMagicHeader, silkDecode, silkEncode, silkIsSilk } from "./silk-wasm-loader.js";
...
function isSilkFile(filePath: string): boolean {
  try {
    ...
    const raw = stripAmrHeader(buf);
    return hasSilkMagicHeader(
...
export async function convertSilkToWav(
  ...
  const result = await silkDecode(rawData, sampleRate);
 ...
 * 
 * UMI Bot API 原生支持 WAV/MP3/SILK 三种格式，其他格式需要先转码。
...
export function shouldTranscodeVoice(filePath: string, mimeType?: string): boolean {
  // MIME 优先：如果 MIME 是 UMI 原生支持的格式，不需要转码
...
export function resolveTTSConfig(cfg: Record<string, unknown>): TTSConfig | null {
  ...
  // 优先使用 channels.umibot.tts（插件专属配置）
  const channelTts = c?.channels?.umibot?.tts;
...
export async function pcmToSilk(
  ...
  const result = await silkEncode(pcmData, sampleRate);
  return {
...
export async function audioFileToSilkBase64(filePath: string, directUploadFormats?: string[]): Promise<string | null> {
  ...
  if ((await silkIsSilk(rawCheck)) || (await silkIsSilk(strippedRaw))) {
