...
// 标准标签名
const VALID_TAGS = ["umiimg", "umivoice", "umivideo", "umifile", 'umimedia'] as const;
...
const TAG_ALIASES: Record<string, typeof VALID_TAGS[number]> = {
  ...
  "doc": "umifile",
  "document": "umifile",
    // ---- umimedia 变体（统一标签，根据扩展名自动路由） ----
  "umi_media": "umimedia",
  "media": "umimedia",
  "attachment": "umimedia",
  "attach": "umimedia",
  "umiattachment": "umimedia",
  "umi_attachment": "umimedia",
  "umisend": "umimedia",
  "umi_send": "umimedia",
  "send": "umimedia",
};
...
/**
 * 自闭合属性语法的正则：
 *   <umimedia file="/path/to/file.png" />
 *   <umiimg src="/path" />
 *   <image file="..." />
 *   <umimedia type="file" path="/path/to/file.zip" />   ← 多属性
 ...
 *  9. 自闭合属性语法：<umimedia file="/path" /> (由 SELF_CLOSING_TAG_REGEX 处理)
 */
...
export function normalizeMediaTags(text: string): string {
  ...
  // <umimedia file="/path/to/file.png" /> → <umimedia>/path/to/file.png</umimedia>
