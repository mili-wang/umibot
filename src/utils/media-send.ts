 ...
 * 富媒体标签解析与发送队列
 *
 * 提供媒体标签（umiimg / umivoice / umivideo / umifile / umimedia）的检测、
...
export const MEDIA_TAG_REGEX =
  /<(umiimg|umivoice|umivideo|umifile|umimedia|img)>([^<>]+)<\/(?:umiimg|umivoice|umivideo|umifile|umimedia|img)>/gi;
...
export interface MediaSendContext {
  ...
  /** qualifiedTarget（格式 "umibot:c2c:xxx" 或 "umibot:group:xxx"，用于 sendMediaAuto） */
        ...
        try {
          const result = await Promise.race([
            ...
            new Promise<{ channel: string; error: string }>((resolve) =>
              setTimeout(
                () => resolve({ channel: "umibot", error: "语音发送超时，已跳过" }),
                voiceTimeout,
              ),
            ),
          ]);
...
export function stripIncompleteMediaTag(text: string): [safeText: string, hasIncomplete: boolean] {
  ...
  // ---- 媒体标签名判断 ----
  const MEDIA_NAMES = [
    "umi", "img", "image", "pic", "photo", "voice", "audio", "video",
    "file", "doc", "media", "attach", "send", "document", "picture",
    "umivoice", "umiaudio", "umivideo", "umiimg", "umiimage", "umifile",
    "umipic", "umiphoto", "umimedia", "umiattach", "umisend", "umidocument", "umipicture",
  ];
  ...
  for (let i = lastLine.length - 1; i >= 0; i--) {
    ...
    if (searchTag) {
      ...
      // 精确/前缀匹配（闭合标签名可能不完整，如 </umi 对 <umivoice）
