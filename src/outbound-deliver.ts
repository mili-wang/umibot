 ...
 * 1. parseAndSendMediaTags — 解析 <umiimg/umivoice/umivideo/umifile/umimedia> 标签并按顺序发送
...
export async function parseAndSendMediaTags(
  ...
  const prefix = `[umibot:${account.accountId}]`;
  ...
  const mediaTarget: MediaTargetContext = {
    ...
    roomId: event.type === "c2c" ? event.room_id : undefined,
    logPrefix: prefix,
  };
  ...
  for (const chunk of chunks) {
    try {
      await sendWithRetry(async (token) => {
        ...
        if (event.type === "c2c") {
          return await sendC2CMessage(token, event.senderId, chunk, account.umi6Sn, event.messageId, event.room_id, ref);
