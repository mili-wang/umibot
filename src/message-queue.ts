...
export function createMessageQueue(ctx: MessageQueueContext): MessageQueue {
  ...
  const processOne = async (
    ...
    try {
      ...
      log?.error(`[umibot:${accountId}] ${label} error for ${peerId}: ${err}`);
      return false;
    }
  };
  ...
  const drainGroupBatch = async (all: QueuedMessage[], peerId: string): Promise<void> => {
    ...
    for (const cmd of commands) {
      log?.info(`[umibot:${accountId}] Processing command independently for ${peerId}: ${(cmd.content ?? "").trim().slice(0, 50)}`);
    ...
    if (normal.length > 0) {
      ...
      if (normal.length > 1) {
        log?.info(`[umibot:${accountId}] Merged ${normal.length} queued group messages for ${peerId} into one`);
      }
  ...
  const drainUserQueue = async (peerId: string): Promise<void> => {
    ...
    if (activeUsers.size >= maxConcurrentUsers) {
      log?.info(`[umibot:${accountId}] Max concurrent users (${maxConcurrentUsers}) reached, ${peerId} will wait`);
      return;
    }
  ...
  const enqueue = (msg: QueuedMessage): void => {
    ...
    if (queue.length >= maxSize) {
      ...
      if (isGroup && dropped?.senderIsBot) {
        log?.info(`[umibot:${accountId}] Queue full for ${peerId}, dropping bot message ${dropped.messageId}`);
      } else {
        log?.error(`[umibot:${accountId}] Queue full for ${peerId}, dropping oldest message ${dropped?.messageId}`);
      }
    }
    ...
    if (totalEnqueued > globalQueueSize) {
      log?.error(`[umibot:${accountId}] Global queue limit reached (${totalEnqueued}), message from ${peerId} may be delayed`);
    }

    queue.push(msg);
    log?.debug?.(`[umibot:${accountId}] Message enqueued for ${peerId}, user queue: ${queue.length}, active users: ${activeUsers.size}`);
  ...
  const startProcessor = (handleMessageFn: (msg: QueuedMessage) => Promise<void>): void => {
    ...
    log?.info(`[umibot:${accountId}] Message processor started (per-user concurrency, max ${maxConcurrentUsers} users)`);
  };
  ...
  const executeImmediate = (msg: QueuedMessage): void => {
    if (handleMessageFnRef) {
      handleMessageFnRef(msg).catch(err => {
        log?.error(`[umibot:${accountId}] Immediate execution error: ${err}`);
      });
    }
  };
