...
export function createMessageQueue(ctx: MessageQueueContext): MessageQueue {
  ...
  const drainUserQueue = async (peerId: string): Promise<void> => {
    ...
    if (activeUsers.size >= MAX_CONCURRENT_USERS) {
      log?.info(`[umibot:${accountId}] Max concurrent users (${MAX_CONCURRENT_USERS}) reached, ${peerId} will wait`);
      return;
    }
    ...
    try {
      while (queue.length > 0 && !ctx.isAborted()) {
        ...
        try {
          ...
          log?.error(`[umibot:${accountId}] Message processor error for ${peerId}: ${err}`);
        }
      }
    } finally {
  ...
  const enqueue = (msg: QueuedMessage): void => {
    ...
    if (queue.length >= PER_USER_QUEUE_SIZE) {
      ...
      log?.error(`[umibot:${accountId}] Per-user queue full for ${peerId}, dropping oldest message ${dropped?.messageId}`);
    }
  ...
  const startProcessor = (handleMessageFn: (msg: QueuedMessage) => Promise<void>): void => {
    ...
    log?.info(`[umibot:${accountId}] Message processor started (per-user concurrency, max ${MAX_CONCURRENT_USERS} users)`);
  };
