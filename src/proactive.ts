...
function loadKnownUsers(): Map<string, KnownUser> {
  ...
  try {
    ...
    console.error(`[umibot:proactive] Failed to load known users: ${err}`);
  }
...
function saveKnownUsers(users: Map<string, KnownUser>): void {
  try {
    ...
    console.error(`[umibot:proactive] Failed to save known users: ${err}`);
  }
}
...
export function recordKnownUser(user: Omit<KnownUser, "firstInteractionAt">): void {
  ...
  console.log(`[umibot:proactive] Recorded user: ${key}`);
}
  ...
  try {
    ...
    if (imageUrl) {
      try {
        ...
        console.log(`[umibot:proactive] Sent image to ${type}:${to}`);
      } catch (err) {
        console.error(`[umibot:proactive] Failed to send image: ${err}`);
        // 图片发送失败不影响文本发送
      }
    }
    ...
    console.log(`[umibot:proactive] Sent message to ${type}:${to}, id: ${result.id}`);
    ...
    console.error(`[umibot:proactive] Failed to send message: ${message}`);
