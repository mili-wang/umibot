...
export function loadUpgradeGreetingTargetOpenId(accountId: string, appId: string, log?: { info: (msg: string) => void }): string | undefined {
  try {
    ...
    if (fs.existsSync(file)) {
      ...
      if (!data.openid) {
        log?.info(`[umibot:${accountId}] upgrade-greeting-target file found but openid is empty`);
        return undefined;
      }
      if (data.appId && data.appId !== appId) {
        log?.info(`[umibot:${accountId}] upgrade-greeting-target appId mismatch: file=${data.appId}, current=${appId}`);
        return undefined;
      }
      if (data.accountId && data.accountId !== accountId) {
        log?.info(`[umibot:${accountId}] upgrade-greeting-target accountId mismatch: file=${data.accountId}, current=${accountId}`);
        return undefined;
      }
      log?.info(`[umibot:${accountId}] upgrade-greeting-target loaded: openid=${data.openid}`);
      return data.openid;
    } else {
      log?.info(`[umibot:${accountId}] upgrade-greeting-target file not found: ${file}`);
    }
  } catch (err) {
    log?.info(`[umibot:${accountId}] upgrade-greeting-target file read error: ${err}`);
  }
  return undefined;
}
...
export function resolveAdminOpenId(ctx: Pick<AdminResolverContext, "accountId" | "appId" | "log">): string | undefined {
  ...
  if (first) {
    ...
    ctx.log?.info(`[umibot:${ctx.accountId}] Auto-detected admin openid: ${first} (persisted)`);
  }
  return first;
}
...
export function sendStartupGreetings(ctx: AdminResolverContext, trigger: "READY" | "RESUMED"): void {
  (async () => {
    ...
    if (!plan.shouldSend || !plan.greeting) {
      ctx.log?.info(`[umibot:${ctx.accountId}] Skipping startup greeting (${plan.reason ?? "debounced"}, trigger=${trigger})`);
      return;
    }
    ...
    if (!upgradeTargetOpenId) {
      ...
      ctx.log?.info(`[umibot:${ctx.accountId}] Version changed but no upgrade-greeting-target, silently updating marker (trigger=${trigger})`);
      return;
    }

    try {
      ctx.log?.info(`[umibot:${ctx.accountId}] Sending startup greeting to upgrade-requester (trigger=${trigger}): "${plan.greeting}"`);
      const token = await getAccessToken(ctx.appId, ctx.clientSecret, ctx.umi6Sn);
      ...
      ctx.log?.info(`[umibot:${ctx.accountId}] Sent startup greeting to upgrade-requester: ${upgradeTargetOpenId}`);
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
      markStartupGreetingFailed(ctx.accountId, ctx.appId, plan.version, message);
      ctx.log?.error(`[umibot:${ctx.accountId}] Failed to send startup greeting: ${message}`);
    }
  })();
}
