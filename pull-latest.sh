...
# 常量 & 参数
##############################################################################
readonly DEFAULT_REPO="https://github.com/mili-wang/umibot.git"
...
##############################################################################
printf "%b\n" "\033[32m=========================================\033[0m"
printf "%b\n" "\033[32m  Umi Bot 拉取最新源码并更新\033[0m"
printf "%b\n" "\033[32m=========================================\033[0m"
echo ""
...
for app in "${SUPPORTED_CLIS[@]}"; do
    ext_dir="$HOME/.$app/extensions/umibot"
...
if [ -z "$PROJ_DIR" ]; then
    PROJ_DIR="$APP_HOME/extensions/umibot"
    FRESH_INSTALL=true
    echo "  插件   未安装（首次安装）"
else
    echo "  插件   $PROJ_DIR"
...
TMP_DIR="${TMPDIR:-/tmp}/umibot-update-$$"
...
if [ -n "$SAVED_CHANNELS_JSON" ]; then
    if node -e "
        ...
        cfg.channels.umibot = $SAVED_CHANNELS_JSON;
    ...
    echo ""
    printf "%b\n" "\033[33m  ⚠️  首次安装，请配置 Umi Bot 凭据:\033[0m"
    echo "     $CMD channels add --channel umibot --token 'YOUR_APPID:YOUR_SECRET'"
    echo ""
fi
...
echo ""
printf "%b\n" "\033[32m=========================================\033[0m"
printf "%b\n" "\033[32m  ✅ Umi Bot 已更新到 ${NEW_VER}${REMOTE_COMMIT:+ (${REMOTE_COMMIT})}\033[0m"
