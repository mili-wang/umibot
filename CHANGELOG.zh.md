...
- **下载目录按账户/对话隔离**：附件下载路径从统一的 `~/.openclaw/media/umibot/downloads/` 改为 `downloads/{appId}/{peerId}/`，按账户和对话隔离，避免多账户文件互相覆盖。
...
OpenClaw 3.23 在 CLI 启动时引入了严格配置校验——任何 `openclaw` 子命令（包括 `plugins install`、`plugins update`、`gateway stop`）执行前都会校验整个 `openclaw.json`。由于 `channels.umibot` 是本插件注册的（非内置 channel id），当插件尚未加载时执行这些命令会报 `"Config invalid: unknown channel id: umibot"` 直接失败（鸡生蛋问题）。

本版本对所有升级路径进行了 3.23+ 适配：

- **CLI 命令前配置暂存/恢复**：`upgrade-via-npm.sh` 和 `upgrade-via-source.sh` 在执行任何 openclaw CLI 命令前临时移除 `channels.umibot`，完成后恢复。
- **安装前预停 Gateway**：`upgrade-via-source.sh` 在 `plugins install` 前先停止 gateway，防止 chokidar 在配置中间状态（`channels.umibot` 已移除）时触发 restart，避免同样的校验错误。
...
- **频道 API 代理工具 `umibot_channel_api`**：AI 可直接调用 UMI 开放平台频道 HTTP 接口，自动 Token 鉴权，内置 SSRF 防护。支持频道/子频道管理、成员查询、论坛发帖、公告发布、日程管理等操作。
...
- **`switchPluginSourceToNpm` 写后校验**：写回 `openclaw.json` 前验证 `channels.umibot` 数据未被破坏，防止竞态写入导致凭证丢失。
...
- **统一富媒体标签**：将 `<umiimg>`、`<umivoice>`、`<umifile>`、`<umivideo>` 统一为 `<umimedia>` 标签，系统根据文件扩展名自动识别媒体类型。
...
- 新增 UMI `REFIDX_*` 引用消息上下文链路：从入站事件解析引用索引，缓存入站/出站消息摘要，并将引用内容注入 agent 上下文。
- 新增引用索引持久化存储（`~/.openclaw/umibot/data/ref-index.jsonl`）：采用内存缓存 + JSONL 追加写，支持重启恢复、7 天 TTL 淘汰与 compact 压缩。
- 新增结构化引用附件摘要（图片/语音/视频/文件、local path/url、语音转录来源），提升引用回复语义完整性。

### 改进

- 机器人回复在可用时自动挂载对当前用户消息的引用，提升 UMI 会话串联可读性。
...
- 新增 `asr_refer_text` 兜底支持——当 STT 未配置或转写失败时，使用 UMI 平台内置 ASR 文本作为低置信度兜底。
...
- 新增 `npm-upgrade.sh` 脚本，支持通过 npm 包安装和升级插件。
  ...
  - 自动处理通道配置备份/恢复、旧插件清理（包括 `umibot`、`@sliverp/umibot`、`umibot`、"@mili-wang/umibot" 等历史版本）、网关重启。
  - 安装前临时移除 `channels.umibot` 配置，避免 `unknown channel id` 校验错误。
...
- `upgrade-and-run.sh` 升级前后现在会显示 umibot 插件版本号。
...
- 增强调试日志——`channel.ts` 中的日志添加 `[umibot:channel]` 前缀，覆盖账户解析、消息发送和网关启动流程。
- API 日志前缀——所有 API 请求日志现在包含 `[umibot-api:${appId}]` 前缀，方便多实例调试。
...
- 升级脚本现在会在升级过程中自动备份和恢复 umibot 通道配置。
