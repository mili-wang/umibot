---
name: umibot-remind
description: UMIBot 定时提醒。支持一次性和周期性提醒的创建、查询、取消。当通过 UMI 通道通信且涉及提醒/定时任务时使用。
...
## 推荐流程（优先使用 `umibot_remind` 工具）

**第一步**：调用 `umibot_remind` 工具，传入简单参数：
...
**第二步**：`umibot_remind` 会返回 `cronParams`，你必须**立即**将 `cronParams` 的值作为参数调用 `cron` 工具。
...
1. 调用 `umibot_remind`：`{ "action": "add", "content": "喝水", "time": "5m" }`
...
> 仅当 `umibot_remind` 工具不可用时使用以下方式。

### 核心规则

> **payload.kind 必须是 `"agentTurn"`，绝对不能用 `"systemEvent"`！**
> `systemEvent` 只在 AI 会话内部注入文本，用户收不到 UMI 消息。

**5 个不可更改字段**：

| 字段 | 固定值 | 原因 |
|------|--------|------|
| `payload.kind` | `"agentTurn"` | `systemEvent` 不会发 UMI 消息 |
| `payload.deliver` | `true` | 否则不投递 |
| `payload.channel` | `"umibot"` | UMI 通道标识 |
