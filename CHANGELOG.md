...
- **Download directory isolated by account/conversation**: Attachment download path changed from a shared `~/.openclaw/media/umibot/downloads/` to `downloads/{appId}/{peerId}/`, isolating files by account and conversation to prevent multi-account file overwrites.
...
OpenClaw 3.23 introduced strict config validation at CLI startup — any `openclaw` subcommand (including `plugins install`, `plugins update`, `gateway stop`) now validates the entire `openclaw.json` before execution. Since `channels.umibot` is registered by this plugin (not a built-in channel id), running these commands when the plugin is not yet loaded causes `"Config invalid: unknown channel id: umibot"` and the command fails entirely (chicken-and-egg problem).

This release adapts all upgrade paths for 3.23+:

- **Config stash/restore for CLI commands**: `upgrade-via-npm.sh` and `upgrade-via-source.sh` temporarily remove `channels.umibot` from `openclaw.json` before running any openclaw CLI command, then restore it after completion.
- **Gateway pre-stop before install**: `upgrade-via-source.sh` now stops the gateway before `plugins install` to prevent chokidar from triggering a restart on the intermediate config state (with `channels.umibot` removed), which would also hit the validation error.
...
- **Channel API proxy tool `umibot_channel_api`**: AI can call UMI Open Platform channel HTTP APIs directly with automatic Token authentication and SSRF protection. Supports guild/channel management, member queries, forum threads, announcements, schedules, and more.
...
- **`switchPluginSourceToNpm` post-write validation**: Verifies `channels.umibot` data integrity before writing back to `openclaw.json`, preventing race-condition credential loss.
...
- **Unified rich media tag**: Replaced `<umiimg>`, `<umivoice>`, `<umifile>`, `<umivideo>` with a single `<umimedia>` tag — the system auto-detects media type by file extension.
...
- Add quoted-message context pipeline for UMI `REFIDX_*`: parse quote indices from inbound events, cache inbound/outbound message summaries, and inject quote body into agent context.
- Add persistent quote index store (`~/.openclaw/umibot/data/ref-index.jsonl`) with in-memory cache + JSONL append, restart recovery, 7-day TTL eviction, and compact.
...
- Bot replies now attach quote reference to the user's current message when available, improving threaded conversation readability in UMI.
...
- Add `asr_refer_text` fallback support — when STT is not configured or fails, use UMI platform's built-in ASR text as low-confidence fallback.
...
- Add `npm-upgrade.sh` script for npm-based plugin installation and upgrade.
  ...
  - Handles channel config backup/restore, old plugin cleanup (including legacy variants like `umibot`, `@sliverp/umibot`), and gateway restart.
  - Temporarily removes `channels.umibot` before install to avoid `unknown channel id` validation error.
...
- `upgrade-and-run.sh` now displays umibot plugin version before and after upgrade.
...
- Enhanced debug logging — `[umibot:channel]` prefixed logs in `channel.ts`, covering account resolution, message sending, and gateway startup.
...
- Upgrade script now auto-backs up and restores umibot channel config during upgrades.
