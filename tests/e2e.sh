...
assert_success "channel.bot1_add" \
  openclaw channels add --channel umibot --token "${BOT1_APPID}:${BOT1_SECRET}" || true
...
assert_success "channel.bot2_add" \
  openclaw channels add --channel umibot --account bot2 --token "${BOT2_APPID}:${BOT2_SECRET}" || true
...
echo "$CONFIG_JSON" | jq '.channels.umibot // empty' 2>/dev/null || echo "(jq parse failed)"
...
MSG_OUT1=$(assert_success "message.bot1_text_c2c" \
  openclaw message send \
    --channel "umibot" \
    --target "umibot:c2c:${BOT1_TEST_OPENID}" \
...
MSG_OUT2=$(assert_success "message.bot2_text_c2c" \
  openclaw message send \
    --channel "umibot" \
    --account bot2 \
    --target "umibot:c2c:${BOT2_TEST_OPENID}" \
