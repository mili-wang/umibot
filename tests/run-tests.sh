...
for VERSION in "${VERSIONS[@]}"; do
  ...
  CONTAINER_NAME="umibot-e2e-${VERSION//[^a-zA-Z0-9_.-]/-}"
  ...
  docker run --rm \
    ...
    -v "$REPORT_DIR:/workspace/umibot/tests/reports" \
    "umibot-e2e:${VERSION}" \
    || EXIT_CODE=$?
...
for VERSION in "${VERSIONS[@]}"; do
  ...
  [ -z "$VERSION" ] && continue
  docker rmi "umibot-e2e:${VERSION}" 2>/dev/null && \
    echo -e "  removed image umibot-e2e:${VERSION}" || true
done
