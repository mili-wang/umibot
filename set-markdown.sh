...
set_markdown_value() {
    ...
    node -e "
      ...
      cfg.channels.umibot.markdownSupport = $value;
...
enable_markdown() {
    ...
    echo ""
    echo "Markdown 已启用。"
    echo "⚠️  请确保您已在 Umi 开放平台申请了 Markdown 消息权限。"
}
...
show_status() {
    ...
    if [ -f "$OPENCLAW_CONFIG" ]; then
        current=$(node -e "
          ...
          console.log(cfg.channels?.umibot?.markdownSupport ?? '未设置');
        " 2>/dev/null || echo "未设置")
